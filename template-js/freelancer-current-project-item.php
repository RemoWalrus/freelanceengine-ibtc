<script type="text/template" id="freelancer_current_project_template_js">
    ...
    <div class="fre-table-col project-average-col"><span><?php _e( 'Average Application', ET_DOMAIN ); ?></span>{{= bid_average}}</div>
    ...
    <div class="fre-table-col project-action-col">
        ...
        <# } else if ( post_status === 'unaccept' ) { #>
            <p><i>
	           <?php _e( 'Your Application is not accepted', ET_DOMAIN ); ?>
            </i></p>
        <# } else if ( post_status === 'publish' ) { #>
            <a class="bid-action" data-action="cancel" data-bid-id="{{= ID }}"><?php _e( 'Cancel Application', ET_DOMAIN ); ?></a>
