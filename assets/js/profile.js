(function($, Models, Collections, Views) {
    ...
    Views.Profile = Backbone.View.extend({
        ...
        openModalAddVideo: function(event) {
            event.preventDefault();
            ...
            fileFrame.on("select", function() {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    data: {
                        action: 'lp_upload_video',
                        filename: filename,
                        file_url: file_url,
                        file_id: file_id
                    },
                    beforeSend: function () {
                        $('#add_video_wrapper').hide();
                        $('#add_video_loader').show();
                    },
                    success: function (data) {
                        if (data.success) {
                            ...
                            $('#add_video_wrapper').show();
                            $('#add_video_loader').hide();
                        }
                    }
                });
            });
            fileFrame.on('open', function(){
                ...
                var attachment_ids = jQuery('#profile_video_id').val();
                selection.add(wp.media.attachment(attachment_ids));
            });
            fileFrame.open();
        },
        openModalRemoveVideo: function(event) {
            event.preventDefault();
            var cnf = confirm("Are you sure you want to remove video?");
            if(cnf){
                $.ajax({
                    type: "post",
                    url: ae_globals.ajaxURL,
